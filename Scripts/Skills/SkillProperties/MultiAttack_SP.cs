...
{
    public class MultiAttack : SkillProperty
    {
        public bool percent;
        ...
        {
            string p = percent ? "%" : "";
            return $"Deals {inflict}{p} {dmgType.ToString()} damage up to {hitCount} times.";
        }
        ...
        {
            return $"MultiAttack$percent>{percent}$dmgType>{dmgType.ToString()}$inflict>{inflict}$hitCount>{hitCount}";
        }
    }
}
